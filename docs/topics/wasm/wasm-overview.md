...
standardized interfaces and types. The Component Model lets you define language-agnostic components in applications
or libraries. You can also dynamically compose Wasm modules and existing components into new components.

To explore what's possible with the Component Model, check out this demo of [a simple server built with `wasi:http`](https://github.com/Kotlin/sample-wasi-http-kotlin/).
